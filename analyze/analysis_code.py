...
# url: sonar-scanner.bat -D"sonar.projectKey=wherecode" -D"sonar.sources=." -D"sonar.host.url=http://localhost:9000" -D"sonar.login=Your login token" -D"sonar.inclusions=**/*.py"
# Define the command with the necessary variables
host_url = sonarqube_server  # SonarQube server url
login_token = sonarqube_token  # SonarQube login token
...
def analyze_gpt_generated_code_and_more(llm_name, language, keyword, start=None, end=None, is_save_code=False,
                                        is_diff=False,
                                        is_download_file=False, is_download_commit=False, is_download_project=False,
                                        is_sonarqube=False):
    """
...
def api_metrics(project_key):
    # API URL
    url = f"{host_url}/api/measures/component"
...
def api_delete_project(project_key):
    url = f"{host_url}/api/projects/delete"
...
def is_analysis_finished(project_key):
    url = f"{host_url}/api/ce/component"
...
def splice_filter_project(llm_name, language):
    ...
    if df_list:
        ...
        filtered_data = all_data[(all_data['create_time'] >= '2023-02-01') & (all_data['project_locs'].notna()) & (
                    all_data['code_language'] != 'Unknown')]
...
def statistic_code_function_type(llm_name, language, only_bug_change=False, top_percent=None):
    ...
    merge_dict = {
        'Program input code, i.e., variable assignments and regular expressions': [
            'Program input code, i.e., variable assignments'],
    ...
    output_filepath = Path(config[llm_name][language]['visualization']['src']) / filename.replace(".csv",
                                                                                                  f"_type_counts_bug_change_{only_bug_change}_percent_{top_percent}.csv")
...
def calculate_various_measures(llm_name, language, columns=('contributor', 'star', 'fork', 'issues', 'watch',
                                                            ...
                                                            'code_cognitive_complexity_all',
                                                            'number_of_all_change_commit', 'final_loc_add',
                                                            'final_loc_minus'), only_change=False, top_percent=None):
    """
        ...
        for index, row in df.iterrows():
            if cal_complexity:
                ...
                if num_code_functions == 0:
                    ...
                    df.at[
                        index, "code_cognitive_complexity_mean_method"] = num_cognitive_complexity_all * 1. / num_code_functions
